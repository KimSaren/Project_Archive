...
int main(int argc, char *argv[]) {
  Date one(17,2,1996);
  Date two(26,7,1997);
  Date three(17,2,1996);

  Player yksi(8480222, 2, "Aho, Sebastian", "New York Islanders", 'D', "SWE", one);
  Player kaksi(8478427,12,"Aho, Sebastian", "Carolina Hurricanes", 'C', "FIN", two);
  

  cout << one << " < " << two << " : " << bool(one < two) << endl;
  cout << two << " < " << one << " : " << bool(two < one) << endl;
  cout << "!(one < two): " << (bool)!(one < two) << endl;
  cout << "!(two < one): " << (bool)!(two < one) << endl;
  cout << "!(one<three) && !(three<one): " << (bool)(!(one<three) && !(three<one)) << endl;

  cout << yksi << " < " << kaksi << " : " << bool(yksi < kaksi) << endl;
  cout << kaksi << " < " << yksi << " : " << bool(kaksi < yksi) << endl;
